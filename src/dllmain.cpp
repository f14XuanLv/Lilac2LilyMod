...
public:
    ...
    void on_unreal_init() override {
        ...
        std::wstring mod_dir = std::filesystem::path{dll_dir}.parent_path().wstring();
        if (mod_dir.empty()) {
            RC::Output::send(STR("[Lilac2LilyMod] failed to resolve mod dir\n"));
            return;
        }

        rt_on_unreal_init(mod_dir.c_str());
    }
};
