...
bool set_imported_size_from_image(UObject* texture, uint32_t width, uint32_t height) noexcept {
    if (width > static_cast<uint32_t>(INT32_MAX) || height > static_cast<uint32_t>(INT32_MAX)) return false;
    const std::wstring value = L"(X=" + std::to_wstring(width) + L",Y=" + std::to_wstring(height) + L")";
    return import_text_property(texture, L"ImportedSize", value.c_str());
}
...
bool private_platform_data_offset(UObject* texture, int32_t& out_offset) noexcept {
    ...
    if (imported_size->GetSize() != 8) {
        log_line("[Lilac2LilyMod][texture2d_forge] ImportedSize is not an 8-byte FIntPoint; cannot infer PrivatePlatformData offset\n");
        return false;
    }
    ...
    out_offset = static_cast<int32_t>(align_up(static_cast<size_t>(imported_offset) + imported_size->GetSize(), alignof(void*)));
    return true;
}
...
bool attach_texture2d_png_platform_data(const UObjectCopyForgeRequest& request, const wchar_t* png_path) noexcept {
    ...
    if (!set_imported_size_from_image(texture, image.width, image.height)) {
        log_line("[Lilac2LilyMod][texture2d_forge] failed to set ImportedSize from decoded PNG dimensions\n");
        return false;
    }
