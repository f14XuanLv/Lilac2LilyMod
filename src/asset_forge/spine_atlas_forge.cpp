...
bool asset_forge_add_spine_atlas(const UObjectCopyForgeRequest& request,
    ...
    try {
        ...
        std::filesystem::path atlas_file{json_path};
        atlas_file.replace_extension(L".atlas");
        std::string atlas_raw = slurp(atlas_file);
        if (atlas_raw.empty()) {
            log_line("[Lilac2LilyMod][spine_atlas_forge] failed: .atlas file missing or empty\n");
            ok = false;
        } else if (!import_text_property(atlas, L"rawData", atlas_raw)) {
            log_line("[Lilac2LilyMod][spine_atlas_forge] rawData import from .atlas failed\n");
            ok = false;
        }
