...
bool map_fstring_animation(UE_FString* name, const char* log_tag) noexcept {
    ...
    const char* tag = log_tag ? log_tag : "spine";
    if (!dst) {
        logf("[Lilac2LilyMod][%s][animap] %s -> (unmapped)\n", tag, src);
        return false;
    }
    if (std::strcmp(src, dst) == 0) {
        logf("[Lilac2LilyMod][%s][animap] %s -> %s (same, no rewrite)\n", tag, src, dst);
        return false;
    }
