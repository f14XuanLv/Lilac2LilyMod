...
// #include <Unreal/Core/HAL/PlatformProcess.hpp>
#include <Unreal/Core/HAL/ThreadSafeCounter.hpp>
/*#include <Unreal/Core/Logging/LogMacros.hpp>*/
/*#include <Unreal/Core/Misc/AssertionMacros.hpp>*/
/*#include <Unreal/Core/Misc/NoopCounter.hpp>*/
...
namespace RC::Unreal
{
    ...
    #else
        UE_FORCEINLINE_HINT void TestCriticalStall()
        {
        }
    #endif
    ...
    public:
        ...
        inline uint32 Alloc(uint32 Count = 1)
        {
        ...
        [[nodiscard]] inline T* GetItem(uint32 Index)
        {
            if (!Index)
            {
                return nullptr;
    ...
    struct FIndexedPointer
    {
        ...
        inline void SetAll(uint32 Ptr, uint64 CounterAndState)
        {
        ...
        [[nodiscard]] UE_FORCEINLINE_HINT uint32 GetPtr() const
        {
        ...
        [[nodiscard]] UE_FORCEINLINE_HINT uint64 GetCounterAndState() const
        {
        ...
        inline void AdvanceCounterAndState(const FIndexedPointer &From, uint64 TABAInc)
        {
        ...
        [[nodiscard]] UE_FORCEINLINE_HINT uint64 GetState() const
        {
        ...
        [[nodiscard]] UE_FORCEINLINE_HINT bool operator==(const FIndexedPointer& Other) const
        {
    ...
    struct FLockFreeLinkPolicy
    {
        ...
        [[nodiscard]] static UE_FORCEINLINE_HINT FIndexedLockFreeLink* DerefLink(uint32 Ptr)
        {
            return LinkAllocator.GetItem(Ptr);
        }
        [[nodiscard]] static UE_FORCEINLINE_HINT FIndexedLockFreeLink* IndexToLink(uint32 Index)
        {
            return LinkAllocator.GetItem(Index);
        }
        [[nodiscard]] static UE_FORCEINLINE_HINT uint32 IndexToPtr(uint32 Index)
        {
            return Index;
        }
    ...
    public:
        ...
        [[nodiscard]] inline uint64 GetState() const
        {
            TDoublePtr LocalHead;
            LocalHead.AtomicRead(Head);
    ...
    // Legacy alias used by FUObjectArray::ObjAvailableList in UE < 4.27
    template<class T>
    using TLockFreePointerList = TLockFreePointerListLIFO<T>;
